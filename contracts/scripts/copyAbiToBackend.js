...
jsonFiles.forEach(jsonFile => {
  try {
    ...
    const outputPath = path.join(targetDir, `${contractName.toLowerCase()}.json`);
    ...
    copiedCount++;
    console.log(`ABI copied to backend: ${outputPath} with address ${address}`);
  } catch (error) {
    console.error(`Error copying ABI from ${jsonFile}:`, error);
  }
});

console.log(`Successfully copied ${copiedCount} contract ABIs to backend`);